...
const TIMELINE: { when: string; what: string; status: 'nuevo' | 'pronto' | 'prep'; icon: GiftIconName }[] = [
  { when: 'Esta semana', what: 'Nuevos stickers ilustrados', status: 'nuevo', icon: 'sticker' },
  { when: 'Próximamente', what: 'Planner edición especial', status: 'pronto', icon: 'notebook' },
  { when: 'Muy pronto', what: 'Kits de regalo', status: 'pronto', icon: 'gift' },
  { when: 'En preparación', what: 'Nueva colección de papelería', status: 'prep', icon: 'palette' },
];
...
export function NovedadesPage({ newProducts, onProductClick, onNavigate, onNotify }: NovedadesPageProps) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => setTimeLeft(getTimeLeft(NEXT_DROP_DATE.getTime())), 1000);
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.hero}>
        <Flower className={`${styles.deco} ${styles.heroFlower}`} color="var(--olffy-naranjo)" size={62} />
        ...
        <div className={styles.heroInner}>
          ...
          <p className={styles.heroSubtitle}>
            Descubre colecciones, productos recién llegados y pequeños adelantos del mundo OLFFY.
          </p>
        </div>
      </section>
      ...
      <div className={styles.wrap}>
        ...
        <Reveal>
          <section className={styles.dropSection}>
            <span aria-hidden="true" className={`${styles.dropBlob} ${styles.dropBlobA}`} />
            <span aria-hidden="true" className={`${styles.dropBlob} ${styles.dropBlobB}`} />
            ...
            <div className={styles.dropGrid}>
              ...
              >
                ...
                <span className={styles.peekHidden} aria-hidden={peekRevealed}>
                  <span className={styles.peekMystery}>
                    <GiftIcon name="notebook" size={34} color="rgba(255,255,255,0.55)" />
                    <GiftIcon name="sticker" size={26} color="rgba(255,255,255,0.4)" />
                    <GiftIcon name="pen" size={30} color="rgba(255,255,255,0.5)" />
                  </span>
                  <span className={styles.peekQuestion}>Colección secreta</span>
                  <span className={styles.peekHint}>Pasa el cursor o toca para ver una pista</span>
                </span>

                <span className={styles.peekReveal} aria-hidden={!peekRevealed}>
                  <Sparkle className={styles.peekSparkle} color="var(--olffy-amarillo)" size={20} />
                  <span className={styles.peekRevealTag}>Una pista...</span>
                  <span className={styles.peekRevealTitle}>Nueva colección de planners ilustrados</span>
                  <span className={styles.peekRevealNote}>Pronto en OLFFY. No podemos contar más todavía.</span>
                </span>
              </button>
              ...
              <div className={styles.countCol}>
                ...
                <h2 className={styles.countTitle}>
                  {timeLeft ? 'Cuenta regresiva para el nuevo drop' : 'La nueva colección ya está disponible'}
                </h2>
                ...
                {timeLeft ? (
                  ...
                  <button type="button" className={styles.countCta} onClick={() => onNavigate('tienda')}>
                    Ir a la tienda
                  </button>
                )}
              </div>
        ...
        <section className={styles.section}>
          <Reveal>
            <div className={styles.sectionHead}>
              ...
              <p className={styles.sectionSub}>Productos que acaban de llegar a la tienda.</p>
            </div>
          </Reveal>
          <Reveal delay={80}>
        ...
        <section className={styles.section}>
          <Reveal>
            <div className={styles.sectionHead}>
              ...
              <h2 className={styles.sectionTitle}>Colecciones para mirar de cerca</h2>
            </div>
          </Reveal>
              ...
              <Reveal key={c.title} delay={i * 90}>
                ...
                >
                  <div
                    ...
                    style={{ background: `linear-gradient(150deg, ${c.bg}, ${c.grad})` }}
                  >
                    <span className={styles.collectionChip}>{c.chip}</span>
                    <GiftIcon name={c.icon} size={46} color={c.accent} />
                  </div>
                  <div className={styles.collectionBody}>
                    <span className={styles.collectionCount} style={{ color: c.accent }}>
                      {c.count}
                    </span>
        ...
        <section className={styles.section}>
          <Reveal>
            <div className={styles.sectionHead}>
              ...
              <h2 className={styles.sectionTitle}>Calendario de lanzamientos</h2>
            </div>
          </Reveal>
          <Reveal delay={80}>
              ...
              {TIMELINE.map((item) => (
                <li key={item.what} className={styles.timelineItem}>
                  <span className={styles.timelineDot}>
                    <GiftIcon name={item.icon} size={17} color="var(--olffy-morado)" />
                  </span>
                  <div className={styles.timelineBody}>
                    <span className={`${styles.timelineChip} ${styles[`chip_${item.status}`]}`}>
        ...
        <Reveal>
          <section className={styles.notifySection}>
            <Sparkle className={`${styles.deco} ${styles.notifySparkle}`} color="var(--olffy-amarillo)" size={20} />
            <div className={styles.notifyInner}>
              <span className={styles.notifyIcon}>
                <GiftIcon name="mail" size={22} color="var(--olffy-morado)" />
              </span>
              <h2 className={styles.notifyTitle}>¿Quieres enterarte primero?</h2>
              <p className={styles.notifyText}>
                Déjanos tu correo y te avisamos cuando lancemos nuevas colecciones.
              </p>

              {emailSent ? (
                <p className={styles.notifySuccess}>Listo, te avisaremos cuando haya novedades.</p>
              ) : (
              ...
              {emailError && !emailSent && (
                <p className={styles.notifyError}>Escribe un correo válido para avisarte.</p>
              )}
            </div>
          </section>
        </Reveal>
