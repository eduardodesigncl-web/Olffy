...
// Foto lifestyle real de la tienda OLFFY (con fallback discreto si falla).
const STORY_IMAGE_SRC = "/olffy/lifestyle/life-2.webp";
...
const STAMPS: ValueStamp[] = [
  {
    icon: "palette",
    label: "Diseño ilustrado",
    detail: "hecho a mano",
    accent: "var(--olffy-naranjo)",
  },
  {
    icon: "notebook",
    label: "Papelería con propósito",
    detail: "para crear y organizar",
    accent: "var(--olffy-morado)",
  },
  {
    icon: "heart",
    label: "Hecho con amor",
    detail: "desde Viña del Mar",
    accent: "#2e7d32",
  },
];
...
export function StorySection({ onNavigate }: StorySectionProps) {
  ...
  return (
    <section className={styles.section} aria-labelledby="story-title">
      ...
      <Sparkle
        className={`${styles.bgDeco} ${styles.bgSparkleA} ${styles.float}`}
        color="var(--olffy-amarillo)"
        size={24}
      />
      <Sparkle
        className={`${styles.bgDeco} ${styles.bgSparkleB} ${styles.floatSlow}`}
      ...
      <div className={styles.canvas}>
        ...
        <div className={styles.photoCol}>
          <div className={styles.photoFrame}>
            {imageOk ? (
              <img
                ...
                src={STORY_IMAGE_SRC}
                alt="Clienta explorando cuadernos ilustrados OLFFY en la tienda de papelería creativa en Viña del Mar, Chile."
                loading="lazy"
          ...
          <Flower
            className={`${styles.photoFlower} ${styles.floatSlow}`}
            color="var(--olffy-naranjo)"
            size={64}
          />
          <Sparkle
            className={`${styles.photoSparkle} ${styles.float}`}
            color="var(--olffy-amarillo)"
            size={20}
          />
        </div>
