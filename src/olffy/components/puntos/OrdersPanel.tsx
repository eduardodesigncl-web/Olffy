...
export type OrderStatus = 'recibido' | 'preparacion' | 'listo_retiro' | 'enviado' | 'entregado';
...
const STEPS_ENVIO: { estado: OrderStatus; label: string; detail: string }[] = [
  { estado: 'recibido', label: 'Pedido recibido', detail: 'Recibimos tu pedido y tu pago.' },
  { estado: 'preparacion', label: 'En preparación', detail: 'Estamos preparando tus productos con cariño.' },
  { estado: 'enviado', label: 'En paquetería', detail: 'Tu pedido va en camino con el transportista.' },
  { estado: 'entregado', label: 'Entregado', detail: 'Tu pedido llegó a destino.' },
];

const STEPS_RETIRO: { estado: OrderStatus; label: string; detail: string }[] = [
  { estado: 'recibido', label: 'Pedido recibido', detail: 'Recibimos tu pedido y tu pago.' },
  { estado: 'preparacion', label: 'En preparación', detail: 'Estamos preparando tus productos con cariño.' },
  { estado: 'listo_retiro', label: 'Listo para retiro', detail: 'Puedes pasar a buscarlo a la tienda.' },
  { estado: 'entregado', label: 'Retirado', detail: 'Entregado en tienda.' },
];

type StepState = 'done' | 'current' | 'pending';

function stepsFor(order: Order): { label: string; detail: string; state: StepState }[] {
  const steps = order.entrega === 'envio' ? STEPS_ENVIO : STEPS_RETIRO;
  const idx = steps.findIndex((s) => s.estado === order.estado);
  return steps.map((s, i) => ({
    label: s.label,
    detail: s.detail,
    state: i < idx || order.estado === 'entregado' ? 'done' : i === idx ? 'current' : 'pending',
  }));
}
...
function OrderHelp({ orderNumber }: { orderNumber: string }) {
  ...
  return (
    <form className={styles.helpBox} onSubmit={handleSubmit} noValidate>
      <h4 className={styles.helpTitle}>¿Tuviste algún problema con tu pedido?</h4>
      <p className={styles.helpText}>Cuéntanos qué pasó y te responderemos por correo.</p>
      ...
      <textarea
        ...
        aria-invalid={error}
      />
      {error && <span className={styles.helpError}>Escribe tu comentario para poder ayudarte.</span>}
      <div className={styles.helpFoot}>
        <button type="submit" className={styles.helpBtn}>Enviar comentario</button>
        <span className={styles.helpNote}>Se enviará al equipo OLFFY ({SUPPORT_EMAIL}).</span>
      </div>
    </form>
  );
}
...
export function OrdersPanel({ orders }: OrdersPanelProps) {
  ...
  if (orders.length === 0) {
    return <div className={styles.empty}>Todavía no tienes pedidos registrados.</div>;
  }
          ...
          <article key={order.id} className={styles.card}>
            ...
            >
              <span className={styles.headIcon}>
                <GiftIcon name="package" size={20} color="var(--olffy-morado)" />
              </span>
              <span className={styles.headInfo}>
                <span className={styles.headTitle}>{order.numero}</span>
                <span className={styles.headMeta}>
                  {order.fecha} · {order.total} · {order.entrega === 'retiro' ? 'Retiro en tienda' : 'Envío a domicilio'}
                </span>
              </span>
              <span className={`${styles.status} ${STATUS_CLASS[order.estado]}`}>
            ...
            {isOpen && (
              <div className={styles.body}>
                ...
                <div className={styles.itemsBox}>
                  ...
                  <ul className={styles.items}>
                    {order.items.map((item) => (
                      <li key={item.nombre} className={styles.item}>
                ...
                <div className={styles.trackBox}>
                      ...
                      <li key={step.label} className={`${styles.step} ${styles[`step_${step.state}`]}`}>
                        ...
                        <span className={styles.stepBody}>
                          <span className={styles.stepLabel}>
                            {step.label}
                            {step.state === 'current' && <span className={styles.stepNow}>Ahora</span>}
                          </span>
                  ...
                  {order.entrega === 'envio' && order.tracking && (order.estado === 'enviado' || order.estado === 'entregado') && (
                    <div className={styles.extraBox}>
                      <span className={styles.extraLabel}>Código de seguimiento</span>
                      <span className={styles.trackingCode}>{order.tracking.codigo}</span>
                      <span className={styles.extraMeta}>Transportista: {order.tracking.transportista}</span>
                      {order.tracking.url && (
                        <a className={styles.trackingLink} href={order.tracking.url} target="_blank" rel="noopener noreferrer">
                          Ver seguimiento
                        </a>
                      )}
                    </div>
                  ...
                  {order.entrega === 'retiro' && order.estado === 'listo_retiro' && (
                    <div className={styles.extraBox}>
                      <span className={styles.extraLabel}>Retiro en tienda</span>
                      <span className={styles.extraMeta}>{STORE_ADDRESS_SHORT}</span>
                      <span className={styles.extraMeta}>Horario: {STORE_HOURS}</span>
                    </div>
                  )}

                  {order.estado === 'entregado' && order.entregadoFecha && (
                    <p className={styles.deliveredNote}>Entregado el {order.entregadoFecha}.</p>
                  )}
                </div>

                {/* Ayuda post entrega */}
                {order.estado === 'entregado' && <OrderHelp orderNumber={order.numero} />}
              </div>
            )}
          </article>
        );
