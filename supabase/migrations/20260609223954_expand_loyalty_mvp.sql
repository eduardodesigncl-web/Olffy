...
values ('Regla base OLFFY', 1000, 10, 12, 30);

insert into public.rewards (
  name, description, reward_type, points_cost,
  discount_amount_clp, validity_days
)
values
...
begin
  ...
  insert into public.loyalty_transactions (
    customer_id, transaction_type, points, source,
    external_reference, description, created_by, metadata
  ) values (
    p_customer_id, 'adjusted', p_points, 'manual',
    nullif(trim(p_external_reference), ''), trim(p_reason),
    trim(p_created_by), p_metadata
  ) returning id into transaction_id;

  insert into public.audit_log (
    entity_type, entity_id, action, actor, new_data
  ) values (
    'loyalty_transaction', transaction_id::text, 'manual_adjustment',
    trim(p_created_by),
    jsonb_build_object(
...
begin
  ...
  insert into public.loyalty_transactions (
    ...
    original_transaction.customer_id, 'reversed',
    -original_transaction.points, 'system',
    'reversal:' || original_transaction.id, trim(p_reason),
    trim(p_created_by),
    jsonb_build_object(
  ...
  insert into public.audit_log (
    ...
    'loyalty_transaction', original_transaction.id::text, 'reversed',
    trim(p_created_by), to_jsonb(original_transaction),
    jsonb_build_object(
...
begin
  ...
  insert into public.loyalty_transactions (
    ...
    p_customer_id, 'redeemed', -reward_record.points_cost,
    'reward_redemption',
  ...
  insert into public.reward_redemptions (
    customer_id, reward_id, loyalty_transaction_id, points_spent,
    status, redemption_code, metadata, expires_at
  ) values (
    p_customer_id, p_reward_id, points_transaction_id,
    reward_record.points_cost, 'requested', generated_code,
    p_metadata, expiration
  ) returning id into redemption_id;

  insert into public.audit_log (
    entity_type, entity_id, action, actor, new_data
  ) values (
    'reward_redemption', redemption_id::text, 'created', trim(p_created_by),
    jsonb_build_object(
      'customer_id', p_customer_id,
  ...
  insert into public.email_events (
    ...
    p_customer_id, 'reward_redemption_created', customer_record.email,
    jsonb_build_object(
...
begin
  ...
  insert into public.audit_log (
    ...
    'reward_redemption', p_redemption_id::text, 'status_changed',
    trim(p_created_by), to_jsonb(redemption_record), to_jsonb(updated_record)
  );
...
begin
  ...
  insert into public.loyalty_transactions (
    ...
    redemption_record.customer_id, 'reversed', redemption_record.points_spent,
    'reward_redemption', 'redemption_cancel:' || redemption_record.id,
    trim(p_reason), trim(p_created_by),
    jsonb_build_object(
  ...
  insert into public.audit_log (
    ...
    'reward_redemption', p_redemption_id::text, 'cancelled',
    trim(p_created_by), to_jsonb(redemption_record),
    jsonb_build_object(
      'reversal_transaction_id', reversal_id,
      'reason', trim(p_reason)
    )
...
begin
  ...
  insert into public.physical_sales (
    customer_id, tuu_transaction_id, receipt_number, subtotal,
    discount, total, points_earned, notes, sold_at, created_by, metadata
  ) values (
    p_customer_id, trim(p_tuu_transaction_id),
    nullif(trim(p_receipt_number), ''), p_subtotal, p_discount, p_total,
    p_points_earned, nullif(trim(p_notes), ''), p_sold_at,
    nullif(trim(p_created_by), ''), p_metadata
  ...
  if jsonb_array_length(p_items) > 0 then
    ...
    select sale_id, item ->> 'shopify_product_id',
      ...
      nullif(item ->> 'sku', ''), item ->> 'product_title',
  ...
  if p_points_earned > 0 then
    insert into public.loyalty_transactions (
      customer_id, transaction_type, points, balance_after, source,
      external_reference, physical_sale_id, description, created_by
    ) values (
      p_customer_id, 'earned', p_points_earned, 0, 'physical_sale',
      trim(p_tuu_transaction_id), sale_id,
  ...
  insert into public.audit_log (
    ...
    'physical_sale', sale_id::text, 'created',
...
grant select, insert, update, delete on table public.loyalty_rules to service_role;
grant usage, select on sequence public.loyalty_rules_id_seq to service_role;

revoke all on function public.adjust_loyalty_points(bigint, bigint, text, text, text, jsonb) from public, anon, authenticated;
revoke all on function public.reverse_loyalty_transaction(bigint, text, text) from public, anon, authenticated;
revoke all on function public.redeem_loyalty_reward(bigint, bigint, text, jsonb) from public, anon, authenticated;
revoke all on function public.update_reward_redemption_status(bigint, text, text) from public, anon, authenticated;
revoke all on function public.cancel_reward_redemption(bigint, text, text) from public, anon, authenticated;

grant execute on function public.adjust_loyalty_points(bigint, bigint, text, text, text, jsonb) to service_role;
grant execute on function public.reverse_loyalty_transaction(bigint, text, text) to service_role;
grant execute on function public.redeem_loyalty_reward(bigint, bigint, text, jsonb) to service_role;
grant execute on function public.update_reward_redemption_status(bigint, text, text) to service_role;
grant execute on function public.cancel_reward_redemption(bigint, text, text) to service_role;
