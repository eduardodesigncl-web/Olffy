create table if not exists public.support_conversations (
  ...
  constraint support_conversations_email_normalized
...
create table if not exists public.support_messages (
  ...
  delivery_channel text not null default 'chat'
    check (delivery_channel in ('chat', 'chat_email')),
...
create index if not exists support_conversations_unread_admin_idx
  on public.support_conversations (unread_admin, last_message_at desc)
...
comment on table public.support_conversations is
  'Bandeja privada de consultas de clientes OLFFY. Acceso exclusivo mediante backend service role.';

comment on table public.support_messages is
  'Mensajes de soporte cliente/admin y estado del aviso por correo.';
